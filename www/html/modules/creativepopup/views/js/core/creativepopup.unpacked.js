						...
						if( window._creativePopup.pluginsLoaded.indexOf( curPluginNameSpace ) === -1 &&
							...
							$.ajax({
								url: 	curPlugin.js.indexOf( 'http://' ) === -1 &&
										...
										window._creativePopup.scriptPath + '/plugins/' + curPlugin.js :
										curPlugin.js,
								dataType: 'script',
								success: function(){
				...
				if( window._creativePopup.checkVersions( ls.initializedPlugins[curPluginNameSpace].pluginData.requiredLSVersion, ls.plugin.version ) ){
					if( curPlugin.css && loadPluginStyle ){
						$( '<link rel="stylesheet" href="' + ( curPlugin.css.indexOf( 'http://' ) === -1 &&
									...
									window._creativePopup.scriptPath.replace('/views/js/', '/views/css/') + '/plugins/' + curPlugin.css :
	...
	window._creativePopup.plugins.popup = function( ls, $slider, sliderUID, userSettings ){
		...
		pu.pluginDefaults = {
			...
			keys: {
				...
				popupOverlayFilter: 			['overlay','styleSettings'],
		...
		pu.plugin = {
			...
			setOptions: function(){
				...
				if( ls.o.popupClickThrough ){
					$slider.addClass('cp-click-through');
				}
		...
		pu.overlay = {
			...
			createMarkup: function(){
				...
				pu.$overlay = $( '<div>' ).addClass( 'cp-popup-overlay' ).css({
					...
					backdropFilter: this.styleSettings.overlayFilter
		...
		pu.popup = {
			...
			createMarkup: function(){
				...
				$slider.find(
					'a[href], :input:not([disabled]), button:not([disabled]), [tabindex]:not([tabindex="-1"]), details>summary:not(:disabled)'
				).each((i, el) => el.setAttribute('tabindex', (el.tabindex || 0) + 1000));

				if( pu.plugin.settings.showCloseButton ){
					$(
						`<div class="cp-popup-close-button" role="button" tabindex="1000" aria-label="${CP_i18n.close}" style="${pu.plugin.settings.closeButtonStyle}">`
					).on('keydown.cpCloseOnEnter', event => {
						if (32 === event.keyCode) {
							event.preventDefault();
						}
					}).on('keyup.cpCloseOnEnter', event => {
			...
			show: function( force ){
				...
				if( !ls.slider.state.popupIsVisible ){
					...
					if( !pu.plugin.settings.disableOverlay ){
						...
						// ACCESSIBILITY
						const $main = $(document.querySelector('body > main') || document.getElementById('page'));
						var activePopups = $main.data('activePopups') || 0;

						$main.attr('inert', true).data('activePopups', ++activePopups);
					}
					pu.popup.show();
				}
			},
			...
			hide: function(){
				...
				if( ls.slider.state.popupIsVisible ){
					...
					if( !pu.plugin.settings.disableOverlay ){
						...
						$main.data('activePopups', --activePopups);
						activePopups || $main.removeAttr('inert');
					}
					pu.popup.hide();
				}
			},
